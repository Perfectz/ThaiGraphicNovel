...
await p.getByRole('button',{name:/Continue adventure/}).click();await p.getByRole('button',{name:'Party growth',exact:true}).click();await p.getByRole('button',{name:'Battle practice',exact:true}).click();await p.waitForFunction(()=>!document.querySelector('.bk-loading'));await p.waitForTimeout(2600);
...
await p.getByRole('button',{name:'Party growth',exact:true}).click();await p.getByRole('button',{name:'Battle practice',exact:true}).click();assert.equal((await state(p)).battle.practice,true);
