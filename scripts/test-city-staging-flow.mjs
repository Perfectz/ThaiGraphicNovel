...
try {
 ...
 await page.getByRole('button',{name:'Battle practice',exact:true}).tap();await page.getByRole('button',{name:/^✦ Word arts/}).waitFor();await page.screenshot({path:out+'/battle.png'});await page.getByRole('button',{name:'Retreat',exact:true}).tap();report.battle=true;
